...
async function createBooking(request, env) {
  ...
  if (required.some((key) => typeof input[key] !== 'string' || !input[key].trim()) || input.consent !== true) return json({ error: 'ข้อมูลจองไม่ครบหรือยังไม่ยอมรับข้อตกลง' }, 400, cors);
  const catalog = await env.DB.prepare('SELECT package_id AS id, package_name AS name, package_price AS price, package_kind AS kind FROM package_catalog WHERE package_id = ? AND package_name = ? AND active = 1 LIMIT 1').bind(input.packageId, input.packageName).first();
  if (!catalog || catalog.kind !== input.packageKind || Number(catalog.price) !== Number(input.packagePrice || 0)) return json({ error: 'แพ็กเกจหรือราคาไม่ถูกต้อง' }, 400, cors);
  const serviceMode = catalog.kind === 'call-pack' ? 'call' : 'async';
