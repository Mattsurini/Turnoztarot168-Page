...
function getDb() {
  if (!db) {
    ...
    `);
    const columns = new Set(db.prepare("PRAGMA table_info(bookings)").all().map((column) => column.name));
    if (!columns.has("package_id")) db.exec("ALTER TABLE bookings ADD COLUMN package_id TEXT");
    if (!columns.has("package_price")) db.exec("ALTER TABLE bookings ADD COLUMN package_price REAL");
  }
  return db;
}
...
function rowToBooking(row) {
  ...
  return {
    ...
    queueCode: Number.isFinite(row.queue_number) ? `BR-${String(row.queue_number).padStart(4, "0")}` : null,
...
export function createBooking(input) {
  ...
  const statement = database.prepare(`
    INSERT INTO bookings (
      booking_code, package_name, package_id, package_price, service_mode, display_name, contact, question,
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    const bookingCode = generateBookingCode(new Date(created));
    try {
      statement.run(
        bookingCode, input.packageName, input.packageId || null, input.packagePrice, serviceMode, input.displayName, input.contact,
      ...
      if (!String(error?.message || "").includes("UNIQUE constraint failed") || attempt === 4) throw error;
    }
  }
