...
function code() {
  ...
  return `BR-${day}-${Math.floor(100 + Math.random() * 900)}`;
}
...
function propertiesForBooking(input, bookingCode, queueNumber) {
...
export async function createNotionBooking(input) {
  ...
  const response = await fetch(`${NOTION_API}/pages`, {
    ...
    body: JSON.stringify({ parent: { data_source_id: ds }, properties: propertiesForBooking(input, bookingCode, queueNumber) }),
  });
  if (!response.ok) throw new Error(`Notion queue create failed: ${response.status}`);
  const created = rowToBooking(await response.json());
...
export async function listNotionBookings(statusFilter = "") {
  const pages = await query(statusFilter ? { property: "Queue Status", select: { equals: statusFilter } } : null);
  return pages.map(rowToBooking).sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt)));
}
