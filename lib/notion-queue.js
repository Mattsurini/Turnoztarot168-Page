...
import { generateBookingCode, isManagedBookingCode } from "./booking-code";
...
function checkbox(value) { return { checkbox: value === true }; }
function notificationState(value) { return select(value || "pending"); }
function select(value) { return { select: value ? { name: value } : null }; }
function status(value) { return { status: { name: value } }; }
function queueCode(queueNumber) { return Number.isFinite(queueNumber) ? `BR-${String(queueNumber).padStart(4, "0")}` : null; }
...
function rowToBooking(page) {
  ...
  return {
    ...
    queueCode: queueCode(value(p["Queue Number"])),
    ...
    notificationSent: value(p["Notification Sent"]) === true,
    notificationStatus: value(p["Notification Status"]) || (value(p["Notification Sent"]) === true ? "sent" : "pending"),
    notificationKey: value(p["Notification Key"]) || page.id,
    notificationAttemptedAt: value(p["Notification Attempted At"]),
    consent: true,
...
function propertiesForBooking(input, bookingCode, queueNumber, createdAt) {
  ...
  return {
    ...
    "Notification Sent": checkbox(false),
    "Notification Status": notificationState("pending"),
    "Notification Key": rich(bookingCode),
    "Notification Attempted At": date(null),
...
export async function createNotionBooking(input) {
  ...
  const queueNumbers = existing
    .map((page) => ({ page, number: value(page.properties?.["Queue Number"]), mode: value(page.properties?.["Service Mode"]) }))
    .filter(({ page, number, mode }) => !page.archived && mode !== "call" && Number.isInteger(number) && number > 0)
    .map(({ number }) => number);
  ...
  const created = rowToBooking(provisionalPage);
...
export async function markNotionNotificationSent(pageId) {
  return updateNotionNotification(pageId, "sent");
}

export async function updateNotionNotification(pageId, state) {
  if (!pageId) throw new Error("Missing Notion page ID");
  if (!["pending", "sending", "sent", "failed"].includes(state)) throw new Error("Invalid notification state");
  const response = await fetch(`${NOTION_API}/pages/${pageId}`, {
    method: "PATCH",
    headers: headers(),
    body: JSON.stringify({ properties: {
      "Notification Sent": checkbox(state === "sent"),
      "Notification Status": notificationState(state),
      "Notification Attempted At": date(new Date().toISOString()),
    } }),
  });
  if (!response.ok) throw new Error(`Unable to mark notification sent: ${response.status}`);
