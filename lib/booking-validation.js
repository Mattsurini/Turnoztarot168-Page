const LIMITS = { packageName: 200, displayName: 120, contact: 200, question: 2000, deliveryType: 30, addons: 500, appointmentDate: 10, appointmentTime: 5 };
...
export function validateBookingInput(body, catalogEntry) {
  if (!catalogEntry?.name || !catalogEntry?.kind || catalogEntry.name !== text(body, "packageName", true)) invalid("Invalid package");
