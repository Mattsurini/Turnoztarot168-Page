...
export async function resolveCatalogPackage(packageName, requestedKind) {
  const name = String(packageName || "").trim();
  const kind = String(requestedKind || "").trim();
  if (!name || !DATA_SOURCES[kind]) return null;
  const pages = await queryDataSource(kind);
  return pages.some((page) => publicPackage(page).name === name) ? { name, kind } : null;
}
...
export async function queryReadingStatus(bookingCode) {
